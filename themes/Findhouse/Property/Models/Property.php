...
//                $join->on('bravo_locations.id', '=', $this->table.'.location_id');
////                    ->where('bravo_locations._lft', '>=', $location->_lft)
////                    ->where('bravo_locations._rgt', '<=', $location->_rgt);
//            })->where($this->table.".status", "publish")
//                ->with(['translation'])
//                ->count($this->table.".id");


            $number = parent::with('location')->where($this->table.".status", "publish")
                ->with(['translation'])
        ...
        if ($number > 1) {
            return  __("properties").' : '.$number.'['. $location->_lft.'|'.$location->id.'|'.$location->_rgt.']';
        }
        return $number. __(":number Properties");
    }
